...
int send_to_peer(peer_t *peer) {
    ...
    do {
        // If sending message has completely sent and there messages into queue, why not send them ?
    ...
    do {
        ...
        if (read_count < 0 && errno != EAGAIN && errno != EWOULDBLOCK) {
            ...
            if (total_read > max_len) {
                printf("Message too large and will be chopped. Please try to be storter next time.\n");
                fflush(STDIN_FILENO);
                break;
            }
        }
