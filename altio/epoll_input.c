...
int main(int argc, char *argv[]) {
    ...
    while (numOpenFds > 0) {
        /* 
         * Fetch up to MAX_EVENTS items from the ready list of 
         * the epoll instance.
         */
        ...
        for (j=0; j<ready; j++) {
            ...
            if (evlist[j].events & EPOLLIN) {
                ...
                /* 
                 * After the epoll_wait(), EPOLLIN and EPOLLHUP
                 * may both have been set. But we will only get 
                 * here, and thus close the file descriptor, if 
                 * EPOLLIN was not set. This ensures that all 
                 * outstanding input (possibly more than MAX_BUF
                 * bytes) is consumed (by further loop iterations)
    ...
    printf("All file descriptors are closed; bye\n");
    exit(EXIT_SUCCESS);
}